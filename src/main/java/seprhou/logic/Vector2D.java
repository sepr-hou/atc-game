...
public class Vector2D {
	 ...
	 * 
	 * @param length length of the vector (negative values reverse the
	 *            direction)
	 ...
	 * 
	 * <p>
	 * This method exists because it avoids the square root which makes
	 * {@link #getLength()} slow. If you just need to compare lengths, you can
	 ...
	 * 
	 * <p>
	 * This method always returns angles between -pi and pi.
	 * <p>
	 * Positive angles are assigned to vectors with positive Y values. This
	 * means that if the Y axis points down, increasing the angle moves
	 * clockwise. If the Y axis points up, increasing the angle moves
	 * anticlockwise.
	 * 
	 ...
	 * 
	 * <p>
	 * See {@link #getAngle()} for information about which way this rotates.
	 * 
	...
	public Vector2D changeLength(float newLength) {
		float myLength = this.getLength();

		if (myLength == 0) {
			return Vector2D.XAXIS.multiply(newLength);
		}

		return this.multiply(newLength / myLength);
	}
